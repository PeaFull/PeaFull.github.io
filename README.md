# PeaFull's Notes

## 项目说明

这是一个基于 Hexo 的个人博客项目，当前使用 NexT 主题，源码分支为 `hexo-source-code`，静态站点部署到 GitHub Pages。

日常维护的核心约定：

- 博客源码、Markdown、配置、主题和依赖维护在 `hexo-source-code` 分支。
- Hexo 生成后的静态 HTML/CSS/JS 部署到 `master` 分支。

## 环境准备

建议使用 Node.js 20 或更高版本。首次拉取项目后安装依赖：

```bash
npm install
```

如果希望严格按照 `package-lock.json` 还原依赖，使用：

```bash
npm ci
```

## 分支与部署约定

这个仓库使用源码分支和部署分支分离的方式：

```text
hexo-source-code  博客源码分支，保存 Markdown、配置、主题、依赖等内容
master            GitHub Pages 部署分支，保存 Hexo 生成后的静态文件
```

日常写文章、改配置、更新主题时，都应该在 `hexo-source-code` 分支进行。GitHub 仓库默认分支也应该保持为 `hexo-source-code`，方便进入仓库后直接看到源码、配置和项目说明。

确认当前源码分支：

```bash
git branch --show-current
git switch hexo-source-code
```

部署时不需要手动切到 `master` 分支。执行部署命令后，`hexo-deployer-git` 会把 `public/` 里的生成结果提交并推送到远端 `master` 分支。

当前部署配置在 `_config.yml`：

```yaml
deploy:
  type: git
  repo: https://github.com/PeaFull/PeaFull.github.io.git
  branch: master
```

## 本地预览

清理旧生成文件并启动本地服务：

```bash
npx hexo clean
npx hexo server
```

运行草稿模式，适用于查看不想发布的文章

```bash
npx hexo server --draft
```

默认访问 `http://localhost:4000`。如果端口被占用，可以换一个端口：

```bash
npx hexo server --port 4001
```

遇到页面内容或配置没有更新时，优先执行 `npx hexo clean` 后再重新预览或生成。

## 写作与内容目录

创建一篇新文章：

```bash
npx hexo new "文章标题"
```

创建草稿：

```bash
npx hexo new draft "文章标题"
npx hexo server --draft
npx hexo publish "文章标题"
```

创建独立页面，例如关于页：

```bash
npx hexo new page "about"
```

常见内容目录：

```text
source/_posts/     正式文章
source/_drafts/    草稿
source/about/      关于页
source/tags/       标签页
source/categories/ 分类页
source/uploads/    图片等静态资源
```

文章开头通常包含 Front Matter，常用字段包括 `title`、`date`、`updated`、`categories`、`tags`、`comments`：

```yaml
---
title: 文章标题
date: 2026-07-07 10:00:00
categories:
  - 编程
tags:
  - Hexo
  - 博客
---
```

当前配置开启了 `post_asset_folder: true`。如果文章需要单独放图片，可以把图片放在和文章同名的资源目录里。

## 生成与发布

只生成静态文件：

```bash
npx hexo clean
npx hexo generate
```

生成后的文件在 `public/` 目录。

生成并部署到 GitHub Pages：

```bash
npx hexo deploy --generate
```

如果部署前想先确认生成没有报错，可以先执行 `npx hexo generate`，确认无误后再执行 `npx hexo deploy`。

## 项目备注

- 当前主题为 `next`。
- NexT 主题配置文件是 `_config.next.yml`。
- 日常源码维护使用 `hexo-source-code` 分支，线上静态页面由 `master` 分支承载。
- 当前仓库存在未跟踪的 `themes/next/` 目录；如果后续整理仓库，需要确认是继续使用 npm 依赖里的 `hexo-theme-next`，还是把本地主题目录纳入版本管理。
- `_config.yml` 里的 `url` 当前仍是示例值，正式发布前建议改成真实站点地址。
